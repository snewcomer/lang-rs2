...
pub fn take_while(f: impl Fn(char) -> bool, s: &str) -> (&str, &str) {
    let extracted_end = s
        .char_indices()
...
mod tests {
    ...
    fn extract_alphanumeric_ident() {
        ...
        assert_eq!(extract_ident("let a"), Ok((" a", "let")));
        assert_eq!(extract_ident("let b = foo"), Ok((" b = foo", "let")));
    }
    ...
    #[test]
    fn tag_works() {
        assert_eq!(tag("let", "let foo"), Ok(" foo"));
    }
}
